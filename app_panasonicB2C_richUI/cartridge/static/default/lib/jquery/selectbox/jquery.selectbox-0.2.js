	...
	$.extend(Selectbox.prototype, {
			...
			sbHolder.data("uid", inst.uid).bind("keydown.sb", function (e) {
				...
				if((key >= 48 && key <= 57) || (key >= 65 && key <= 90) || (key >= 96 && key <= 105)){
					if((key >= 48 && key <= 57) || (key >= 65 && key <= 90)){
					 ...
					 $this.find('.sbOptions li').each(function(i){
						var $text = $(this).find('a').text(); 
						var $substring = $text.substring(0,keyvalue.length).toLowerCase();
						if(keyvalue == $substring){
							$this.find('.sbOptions li a').removeClass('sbFocus');
							$(this).find('a').addClass('sbFocus');
		...
		_changeSelectbox: function (target, value, text) {
			...
			$('input[name$="phone"]').each(function(){
				...
				$(this).closest('.customphone-row').find('.first').val(phoneVal.substr(0,3));
				$(this).closest('.customphone-row').find('.middle').val(phoneVal.substr(3,3));
				$(this).closest('.customphone-row').find('.last').val(phoneVal.substr(6,4));
			});
		},
		/**
