...
dw.ext.CustomObjectGridEditor.prototype.initialiseCustomObjectEditor = function() {
	this.store = new dw.ext.CustomObjectStore(this.customObjectType);
			...
			for ( var attributeID in editorScope.objectDefinition) {
				if (!editorScope.objectDefinition[attributeID].isSystem) {
					editorScope.customObjectColumns.push({
						text : editorScope.objectDefinition[attributeID].displayName,
...
dw.ext.CustomObjectGridEditor.prototype.buildMainPanel = function() {
	...
	this.mainPanel = Ext.create('Ext.Panel', {
		...
		bbar : [ "->", {
			xtype : 'button',
			icon : Ext.get('iconPrefixUrl').dom.getAttribute('data-url') + 'accept.png',

			text : 'Apply',
			handler : function() {
				//var currentRecord = editorScope.grid.selModel.getSelection()[0];
				editorScope.formPanel.form.updateRecord();
			...
			text : 'Cancel',
			border : 1,
			icon : Ext.get('iconPrefixUrl').dom.getAttribute('data-url') + 'cancel.png',
			handler : function() {
		...
		tbar : [ {
			text : 'Add Object',
			icon : Ext.get('iconPrefixUrl').dom.getAttribute('data-url') + 'application_form_add.png',
			handler : function() {

				var newObject = Ext.create(editorScope.store.getProxy().getModel().modelName);
			...
			text : 'Remove Object',
			icon : Ext.get('iconPrefixUrl').dom.getAttribute('data-url') + 'application_form_delete.png',
